...
export const LoginPage: React.FC = () => {
  ...
  const showAccessDenied =
    reason === 'not_credit' || Boolean(error && error.includes('Access denied'));
      ...
      <Box className="login-container" component="main">
        <Box className="login-header">
          ...
          <Typography variant="body2" className="subtitle-text">
            Review submitted applications and make credit decisions
          </Typography>
        </Box>
        <Box component="form" className="login-form" onSubmit={handleSubmit(onSubmit)} noValidate>
          {showAccessDenied ? (
            <Alert severity="error" className="login-alert">
              <Typography variant="body2">
                <strong>Access denied.</strong> Credit officer privileges are required.
              </Typography>
            </Alert>
          ) : null}
          <Input
          ...
          <Box className="form-options">
            ...
            <Link component={RouterLink} to="/credit/auth/forgot-password" className="forgot-link">
              Forgot password?
            </Link>
          </Box>
