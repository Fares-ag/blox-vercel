...
export const EditProductPage: React.FC = () => {
  ...
  useEffect(() => {
    supabaseApiService.getCompanies().then((res) => {
      if (res.status === 'SUCCESS' && res.data) {
        setCompanies(res.data.filter((c: Company) => c.status === 'active'));
      }
    });
  }, []);

  const companyOptions: SelectOption[] = [
    { value: '', label: 'None (Blox inventory)' },
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              <Select
                ...
                helperText="A badge with the company name will appear on the vehicle card"
              />
            </Grid>
            <Grid item xs={12}>
              <Input
