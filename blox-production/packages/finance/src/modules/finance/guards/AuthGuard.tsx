...
export const AuthGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  if (!isAuthenticated) {
    return <Navigate to="/finance/auth/login" state={{ from: location }} replace />;
  }
  if (!isFinancePortalRole(user?.role)) {
    return <Navigate to="/finance/auth/login?reason=not_finance" replace />;
  }
  return <>{children}</>;
};
