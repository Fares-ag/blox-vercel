...
const documentCategories = [
  ...
  { id: 'drivers-license', label: '2. Driver\'s License', required: true },
  { id: 'police-certificate', label: '3. Police Certificate (Optional)', required: false },
];
...
export const CreateApplicationPage: React.FC = () => {
  ...
  useEffect(() => {
    const checkExistingApplications = async () => {
      try {
        ...
        // Check applications from Supabase (server-side filtered by email)
        const supabaseResponse = await supabaseApiService.getMyApplications(customerEmail);
        const customerApplications: Application[] = supabaseResponse.status === 'SUCCESS' && supabaseResponse.data
  ...
  const onSubmit = async (data: ApplicationFormData) => {
    ...
    // Load applications from Supabase (server-side filtered by email)
    const appsResponse = await supabaseApiService.getMyApplications(customerEmail);
    const customerApplications: Application[] = appsResponse.status === 'SUCCESS' && appsResponse.data
      ? appsResponse.data
      : [];
    ...
    try {
      ...
      // Fixed amortized monthly payment (same formula as InstallmentCalculator)
      const monthlyRate = annualRentRateDecimal / 12;
      const pow = Math.pow(1 + monthlyRate, termMonths);
      const fixedMonthlyPayment = loanAmount > 0 && termMonths > 0
      ...
      for (let i = 0; i < termMonths; i++) {
        ...
        schedule.push({
          ...
          amount: Math.round(fixedMonthlyPayment * 100) / 100,
          status,
      ...
      const supabaseResponse = await supabaseApiService.createApplication(
        {
          ...
          installmentPlan: {
            tenure: tenureString,
            interval: 'Monthly',
            monthlyAmount: fixedMonthlyPayment || monthlyPayment || 0,
      ...
      if (supabaseResponse.status === 'SUCCESS' && supabaseResponse.data) {
        ...
        // Navigate to confirmation page in all cases
        if (!isAuthenticated) {
          const { data: { user: freshUser } } = await supabase.auth.getUser();
          const emailVerificationRequired = !!(freshUser && !freshUser.email_confirmed_at);
          navigate('/customer/applications/confirmation', {
            state: {
              applicationId,
              email: resolvedCustomerEmail,
              isNewUser: true,
      ...
      <Box component="form" onSubmit={handleFormSubmit} className="application-form">
        ...
        <Card className="form-section">
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary" className="section-subtitle">
              Please upload your Qatar ID and Driver's License. Police Certificate is optional.
            </Typography>
            <Divider sx={{ my: 3 }} />
