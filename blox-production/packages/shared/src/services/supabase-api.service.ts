...
class SupabaseApiService {
  ...
  async getProducts(): Promise<ApiResponse<Product[]>> {
    ...
    try {
      ...
      const products = handleSupabaseResponse<any[]>(response).map((row: any) => {
        const product = mapSupabaseRow<Product>(row);
        if (row.company) {
          product.companyId = row.company.id;
          product.companyName = row.company.name;
        }
        return product;
      });
  ...
  async updateProduct(id: string, product: Partial<Product>): Promise<ApiResponse<Product>> {
    try {
      ...
      if (product.companyId !== undefined) updateData.company_id = product.companyId || null;
  ...
  async getApplications(): Promise<ApiResponse<Application[]>> {
    ...
    try {
      // Use simple select without named FK hints to avoid constraint-name issues across projects
      const response = await supabase
        .from('applications')
        .select('*')
        .order('created_at', { ascending: false });

      const rows = handleSupabaseResponse<any[]>(response);

      // Enrich each application with vehicle and offer data
      const applications = await Promise.all(rows.map(async (app: any) => {
        ...
        if (app.vehicle_id) {
          const { data: vehicleData } = await supabase
            ...
            .maybeSingle();
          if (vehicleData) mapped.vehicle = mapSupabaseRow<Product>(vehicleData);
        }
        ...
        if (app.offer_id) {
          const { data: offerData } = await supabase
            ...
            .eq('id', app.offer_id)
            .maybeSingle();
          if (offerData) mapped.offer = mapSupabaseRow<Offer>(offerData);
        }
  ...
  /**
   * Fetches only the applications belonging to the given customer email.
   * Filters server-side for reliability — use this in the customer portal.
   */
  async getMyApplications(customerEmail: string): Promise<ApiResponse<Application[]>> {
    try {
      ...
      if (response.error) {
        return { status: 'ERROR', message: response.error.message, data: [] };
      }
  ...
  async createApplication(
    ...
    try {
      ...
      const appData: any = {
        ...
        offer_id: application.offerId || null,
