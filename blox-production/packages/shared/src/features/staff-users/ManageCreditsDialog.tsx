...
import { Button as CustomButton } from '../../components/core/Button/Button';
...
export const ManageCreditsDialog: React.FC<ManageCreditsDialogProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    await onSave(
      ...
      numAmount,
      description.trim() ||
        `Admin ${action === 'add' ? 'added' : action === 'subtract' ? 'subtracted' : 'set'} credits`
    );
    handleClose();
  };
      ...
      <DialogContent className="dialog-content">
        ...
        <TextField
          ...
          placeholder={`Reason for ${
            action === 'add' ? 'adding' : action === 'subtract' ? 'subtracting' : 'setting'
          } credits...`}
          value={description}
