...
export const GuestGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  if (location.pathname === '/dealer/auth/reset-password') return <>{children}</>;
  if (isAuthenticated && isDealerPortalRole(user?.role)) {
    return <Navigate to="/dealer/applications" replace />;
  }
  return <>{children}</>;
};
