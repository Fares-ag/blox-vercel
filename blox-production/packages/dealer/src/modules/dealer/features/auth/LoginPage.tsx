...
export const LoginPage: React.FC = () => {
  ...
  const showAccessDenied =
    reason === 'not_dealer' ||
    Boolean(error && error.includes('Access denied'));
      ...
      <Box className="login-container" component="main">
        <Box className="login-header">
          ...
          <Typography variant="body2" className="subtitle-text">
            Showroom applications, deal pricing, and credit submission
          </Typography>
        </Box>
        <Box component="form" className="login-form" onSubmit={handleSubmit(onSubmit)} noValidate>
          {showAccessDenied ? (
            <Alert severity="error" className="login-alert">
              <Typography variant="body2">
                <strong>Access denied.</strong> Dealer agent privileges are required.
              </Typography>
            </Alert>
          ) : null}
          <Input
          ...
          <Box className="form-options">
            ...
            <Link component={RouterLink} to="/dealer/auth/forgot-password" className="forgot-link">
              Forgot password?
            </Link>
          </Box>
