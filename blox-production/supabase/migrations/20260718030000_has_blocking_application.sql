-- Guest apply cannot use applications SELECT under RLS (empty for anon).
-- SECURITY DEFINER returns a blocking application id for an email, or NULL.
...
COMMENT ON FUNCTION public.has_blocking_application(text) IS
  'Returns latest blocking application id for email (guest apply pre-check).';
